...
class Extrude: public meshmaker::DomainBase {
  public:
  Extrude(
      const std::shared_ptr<meshmaker::Polygon2D> & poly,
  ...
  private:
  const std::shared_ptr<meshmaker::Polygon2D> poly_;
...
class ring_extrude: public meshmaker::DomainBase {
  public:
  ring_extrude(
      const std::shared_ptr<meshmaker::Polygon2D> & poly,
